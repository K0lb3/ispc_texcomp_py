...
from pybind11.setup_helpers import Pybind11Extension, build_ext, auto_cpp_level
...
def build():
    setup(
        ...
        cmdclass={"build_ext": build_ext_ispc_support},
        zip_safe=False,
    )


class build_ext_ispc_support(build_ext):
    def build_extension(self, ext):
        build_temp = os.path.realpath(self.build_temp)
        # fetch ispc compiler
        ispc = fetch_ispc_compiler()
        ...
        while i < len(ext.sources):
            if ext.sources[i].endswith(".ispc"):
                ...
                i += 1
        # compile ispc files
        extra_objects = build_ispc(ispc, ispc_files, build_temp)
...
def compile_ispc(ispc, source, arch, targets, build_dir):
    ...
    outputs = [
        os.path.join(build_dir, name + ".o"),
        *[os.path.join(build_dir, name + "_" + target + ".o") for target in targets],
    ]
    ...
    arguments = [
        ...
        f"{build_dir}/{name}.o",
        "-h",
        f"{build_dir}/{name}_ispc.h",
        "--arch={arch}" if arch else "",
        f'--target={",".join(targets)}',
        "--opt=fast-math",
    ]
    code = os.system(f'{ispc} {" ".join(arguments)}')
    if code != 0:
        raise RuntimeError("ISPC compilation failed")
    return outputs


def build_ispc(ispc, ispc_files, build_dir):
...
def fetch_ispc_compiler():
    """Fetches the ispc compiler from the official repository."""
    system = platform.system()
    if system == "Windows":
        ispc_fp = os.path.join(LOCAL, "ISPC", "win", "ispc.exe")
    else:
        ispc_fp = os.path.join(
            LOCAL, "ISPC", "linux" if system == "Linux" else "osx", "ispc"
        )
